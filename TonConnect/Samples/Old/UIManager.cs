...
public class UIManager : MonoBehaviour
{
    ...
    private List<string> WalletsIconsList = new () {"tonkeeper", "tonhub", "openmask",
        "dewallet", "mytonwallet", "tonflow", "tonwallet", "xtonwallet", "telegram-wallet"};
    ...
    private async void SendTXModalSendButtonClick(ClickEvent clickEvent)
    {
...
#if UNITY_2021
        string sendValueString = document.rootVisualElement.Q<TextField>("SendTXModal_Value").value;

        if (!double.TryParse(sendValueString, out double sendValue) || sendValue <= 0)
        {
            return;
        }
#else
        ...
        if (string.IsNullOrEmpty(receiverAddress) || sendValue <= 0)) 
                return;
#endif
    ...
    public void ShowConnectModal()
    {
        ...
        StartCoroutine(tonConnectHandler.BestLoadWallets("https://raw.githubusercontent.com/ton-blockchain/wallets-list/main/wallets-v2.json", LoadWalletsCallback));
    }
